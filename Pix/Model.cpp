...
void Model::Load(const std::string& filename)
{
	...
	while (true)
	{
		...
		{
			uint32_t v[3];
			if (fscanf_s(file, "%d//*%d %d//%*d %d//%*d\n", &v[0], &v[0], &v[1], &v[1], &v[2], &v[2]) != 6)
			{
				char errorBuffer[128];
				sprintf_s(errorBuffer, "Failed to parse face data in file: %s", filename.c_str());
				MessageBoxA(nullptr, errorBuffer, "Error", MB_OK | MB_ICONERROR);
				return;
			}
